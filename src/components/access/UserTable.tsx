    ...
    return (
        <Table className='product-table text-center'>
            ...
            <TableBody>
                {
                    data.map((item, i) =>
                        item.created_at ? <Row item={item} i={i} key={i} /> :
                            <AddRow i={i} />
                    )
                }
            </TableBody>
    ...
    return (
        <TableRow className='cursor-pointer'>
            ...
            <TableCell className='capitalize'>
                {
                    item.role === "admin"
                        ? <Role id={item.id}><p className='font-semibold'>Admin</p></Role>
            ...
            <TableCell>
                {
                    item.verified_at
                        ...
                        :
                        <p className='flex items-center justify-center gap-2'><span>Pending</span>
                            <Tooltip title='Remove User'>
                                <Button size={"sm"} className='w-8 h-8 p-0 bg-rose-500 hover:bg-rose-600 text-white'>
...
const AddRow = ({ i }: { i: number }) => {
    ...
    return (
        <TableRow>
            ...
            <TableCell className='capitalize'>
                <Dropdown label='Access Role' items={["sub-admin", "user"]} handleClick={(option) => { console.log(option) }}>
...
const Badge = ({ variant, className, type }: {
    ...
    return (
        <span className={cn("block min-w-16 max-w-20 items-center px-2.5 py-1 rounded-full text-xs font-medium text-white w-full", variant === "outline" ? "border" : "", type === "active" ? "bg-emerald-500" : type === "inactive" ? "bg-rose-500" : type === "draft" ? "bg-orange-500" : "", className)}>
            {type}
        </span>
    )
}
...
const Role = ({ children, id, type }: {
    ...
    return (
        <Dropdown label='Change Status' items={['sub-admin', 'user']} handleClick={handleClick}>
            <div className='cursor-pointer'>
                {children}
            </div>
        </Dropdown>
