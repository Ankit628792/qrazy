...
type UploadLogoProps = Readonly<{
  file: File | null
  size?: string
  handleChange: (file: File) => void
}>

const UploadLogo = ({ file, size = '', handleChange }: UploadLogoProps) => {
  return (
    <div className={cn('overflow-hidden fileUpload flex')}>
      ...
      >
        <React.Fragment>
          <div
            className={cn(
              'logo p-10  absolute inset-0 bg-white dark:bg-black text-black dark:text-white border rounded-full place-items-center fileUploadOverlay z-20 shrink-0',
              initialSize,
              size
            )}
          >
            <svg
              className="w-full h-full fill-gray-100 dark:fill-zinc-800 stroke-emerald-600"
              viewBox="0 0 501 501"
              fill="none"
            ...
            <p className="text-2xl max-w-max absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              Drop Logo here
            </p>
          </div>
          {file ? (
            <div
              className={cn(
                'rounded-full overflow-hidden bg-white dark:bg-black text-black dark:text-white border grid place-items-center cursor-pointer fileUploadContainer',
                initialSize,
                size
              )}
            >
