        ...
        <>
            ...
            <section className='relative'>
                ...
                <div className='bg-white dark:bg-black bg-opacity-50 dark:bg-opacity-50 backdrop-blur-sm rounded-2xl overflow-hidden z-10 transform -translate-y-72 min-h-96'>
                    <div className='flex items-center w-full overflow-x-auto divide-x scroll-hidden '>
                        {
                            Array(6).fill(1).map((_, i) => {
                                return (
                                    <div key={i} className='glass w-full min-w-80 max-w-xs relative p-5 filter'>
                                        <div className='flex items-start justify-between gap-4'>
                                            <Tooltip title="Show Info Here">
                                                <Info className='w-5 shrink-0' />
                                            </Tooltip>
                                            <img src={i % 2 === 0 ? ChartUp.src : ChartDown.src} className='w-48 h-full object-cover' alt="" />
                                        </div>
                    ...
                    <div className='p-4 flex items-start flex-col-reverse lg:flex-row gap-4 '>
                        <div className='w-full flex-grow rounded-xl glass-base p-4 divide-y'>
                            <h1 className='text-2xl lg:text-3xl font-semibold pb-3'>Recent Scans</h1>
                            <div className='w-full flex flex-col divide-y'>
                                <ScanTable data={dummyScans} />
                            </div>
                        </div>
                        <div className='w-full lg:max-w-sm rounded-xl glass-base p-4 divide-y'>
                            <h1 className='text-2xl lg:text-3xl font-semibold pb-3'>Top Places</h1>
                            <div className='w-full flex flex-col divide-y'>
                                {
                                    Array(10).fill(1).map((_, i) => <Place key={i} />)
                                }
                            </div>
                        </div>
                    </div>
...
const Place = () => {
    return (
        <div className='flex items-center justify-between w-full py-3'>
            <h1 className='flex-grow'>Jammu & Kashmir, Republic of India, Pakistan</h1>
            <div className='text-right shrink-0'>
                <h3 className='font-medium'>156 Scans</h3>
                <p className='text-xs text-gray-500'>last {moment().subtract(7, 'minutes').fromNow()}</p>
            </div>
        </div>
    )
}
