...
function ScannedProduct({
  ...
  return (
    <div
      className={cn(
        'w-full flex-grow flex items-start gap-2 lg:gap-3 rounded-xl p-2 cursor-pointer bg-gray-100 dark:bg-zinc-900 relative',
        className
      )}
    >
      <img
      ...
      <span className="absolute bottom-1 right-2 text-xs text-gray-500">
        Scanned {moment().subtract(2, 'm').fromNow()}
      </span>
    </div>
  )
}
