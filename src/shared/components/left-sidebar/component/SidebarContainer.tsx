  ...
  return (
    <div key={index} className="w-full">
      <Link href={item.link} onClick={() => setSelectedMenuItem(item.title)}>
        <Container
          className={`flex items-center justify-start gap-2 p-2 cursor-pointer hover:bg-gray-200 ${selectedMenuItem === item.title ? 'bg-gray-200 outline-y outline-gray-500/10' : ''}`}
        >
